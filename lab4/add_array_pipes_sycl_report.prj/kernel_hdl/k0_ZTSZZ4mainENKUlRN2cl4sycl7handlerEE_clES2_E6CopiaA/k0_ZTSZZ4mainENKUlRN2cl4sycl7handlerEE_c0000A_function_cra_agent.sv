...
// SystemVerilog created from k0_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE_c0000A_function_cra_agent
...
module k0_ZTSZZ4mainENKUlRN2cl4sycl7handlerEE_c0000A_function_cra_agent (
    ...
    // start_bit(BITSELECT,172)
    assign start_bit_b = status_NO_SHIFT_REG_q[0:0];

    // start_or_start_buffered(LOGICAL,177)
    ...
    // start_buffered_and_kernel_idle(LOGICAL,174)
    assign start_buffered_and_kernel_idle_q = not_running_bit_q & buffered_start_NO_SHIFT_REG_q;

    // start_from_buffered_start_NO_SHIFT_REG(REG,175)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // printf_counter_reset_mux(MUX,153)
    ...
    // printf_bit(BITSELECT,151)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,152)
    ...
    // readdata_valid_stage1_reg(REG,159)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // start_bit_computation(LOGICAL,173)
    ...
    // printf_reset_bit(BITSELECT,154)
    ...
    // readdata_upper_bits_mux(MUX,158)
    ...
    // readdata_valid_stage2_reg(REG,160)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // dupName_2_arg_bit_join_x(BITJOIN,206)
    ...
    // dupName_1_arg_bit_join_x(BITJOIN,201)
    ...
    // dupName_0_arg_bit_join_x(BITJOIN,194)
    ...
    // start_is_or_going_high(LOGICAL,176)
    ...
    // workgroup_size_NO_SHIFT_REG(REG,192)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
